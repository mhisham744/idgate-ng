...
export function buildSeed(): AppData {
  ...
  // ── Notifications (tools) ─────────────────────────────────────────────────────
  const ntTaskTo = { kind: 'virtual' as const, virtualId: virtuals.find((v) => v.entityId === 'e_nestle' && v.positionName === 'Supply Chain Director')!.id }
  ...
  const notifications: Notification[] = [
    ...
    { id: 'nt_2', kind: 'voting', from: { kind: 'virtual', virtualId: moeBoard.id }, to: [ntVoteTo], subject: 'Approve renewable tariff resolution', body: 'Board resolution #2026-14: approve the new residential renewable-energy tariff. Please cast your vote.', createdAt: ago(220), needsResponse: true, status: 'pending', recipients: [{ ref: ntVoteTo, status: 'pending', thread: [] }], frozen: false, votes: { accept: 4, reject: 1 }, history: [] },
